...
xtrain=df1.drop(columns=["y"],axis=1).astype(np.float32).values
ytrain=df1["y"].astype(np.int32).values
...
#train the model
model.fit(xtrain,ytrain, epochs=100, batch_size=32, validation_split=0.2, callbacks=[early_stopping])

#predictions
data=pd.read_csv("/content/sample_data/sample_data.csv")
